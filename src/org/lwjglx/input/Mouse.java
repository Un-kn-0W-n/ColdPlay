...
public class Mouse {
    ...
    private static boolean ignoreFirstDelta;
        ...
        glfwSetCursorPosCallback(Display.getWindow(), new GLFWCursorPosCallback() {
            ...
            public void invoke(long l, double v, double v1) {
                if(Display.isActive()){
                    ...
                    if(ignoreFirstDelta){
                        // First event after a re-grab: GLFW reports a discontinuous jump
                        // (CURSOR_DISABLED switch). Discard it so it isn't applied as rotation.
                        current_dx = 0;
                        current_dy = 0;
                        ignoreFirstDelta = false;
                    }else{
                        // Raw pixel delta; mouse sensitivity is applied once downstream in EntityRenderer.
                        // Accumulate: one glfwPollEvents() delivers every event queued since the last
                        // frame, and getDX/getDY drain once per frame. Overwriting kept only the last
                        // event, silently dropping motion whenever a frame was long (low FPS on join).
                        current_dx += poll_xPos - last_x;
                        current_dy += poll_yPos - last_y;
                    }
                    pollNeed = true;
                }
            }
    ...
    public static void create(){
        ...
        readBuffer.limit(0);
        current_Event = new MouseEvent(); // GUI opening can reposition the cursor before the first OS event.
    ...
    public static void setGrabbed(boolean grab){
        ...
        if(grab){
            // Re-grabbing (e.g. closing a GUI) must not apply leftover mouse motion as rotation.
            // Drain any stale delta accumulated while the cursor was free, re-seed the delta
            // baseline, and ignore the first cursor-pos event after the CURSOR_DISABLED switch.
            current_dx = 0;
            current_dy = 0;
            last_x = poll_xPos;
            last_y = poll_yPos;
