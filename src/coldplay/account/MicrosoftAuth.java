...
/**
 * Refresh-token authentication through Microsoft, Xbox Live, XSTS, and Minecraft Services.
 * Uses login.live.com/MBI_SSL to match pasted launcher refresh tokens; persist the rotated token.
 * Callers run network work off the render thread. The game-server proxy does not apply here.
 */
public final class MicrosoftAuth {
    ...
    /** The minted Minecraft access token plus the rotated refresh token to persist for next time. */
    ...
    public MsaAuthResult loginWithRefreshToken(final String refreshTokenInput) throws IOException {
        ...
        // Microsoft rotates the refresh token; fall back to the one we sent if it didn't return a new one.
