...
/**
 * Global SOCKS5 proxy for game-server connections, independent of the active account.
 * Persists editable fields in {@code <mcDataDir>/coldplay/proxy.json}.
 */
public final class ProxyManager {
    ...
    // Disabling preserves credentials. Older files default to OFF to avoid silently enabling a proxy.
    ...
    /** Lazily loads from the running client's data dir, then returns the active proxy (null = direct or disabled). */
    ...
    /** True when a proxy is saved, whether or not it is enabled (for the account-screen status/toggle). */
    ...
    /**
     * Proxy connections resolve the server host remotely; direct connections use local DNS.
     */
    ...
    /**
     * Sets (or, for blank input, clears) the global proxy and persists it.
     *
     * @throws IllegalArgumentException if the address is malformed (surfaced to the user as a status line)
     */
    ...
    /** On-disk shape of {@code proxy.json}: the raw entered strings. Null proxyEnabled (pre-toggle file) = OFF. */
