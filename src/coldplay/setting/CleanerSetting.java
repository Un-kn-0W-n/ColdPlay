...
/**
 * Persists cleaning modes by item key ("registry:meta") or category ("@cat:Name"). Item modes
 * override category modes; absent entries leave items alone. Uses HotbarSetting's shared registry.
 */
public class CleanerSetting extends AbstractItemPickerSetting {

    // click cycle on a cell: unset -> DROP -> KEEP_ONE -> IGNORE -> unset
    ...
    // the mode that applies to this stack: a per-item override beats its category's mode. null = leave it.
    ...
    public JsonElement toJson() {
        // Sorted key order keeps the {token -> mode} map deterministic on disk.
