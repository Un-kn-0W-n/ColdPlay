...
public class HotbarSetting extends AbstractItemPickerSetting {
    ...
    // bucket ids double as the index into CATEGORIES
    ...
    public static final class Entry {
        ...
        private final boolean subtypes; // meta is a variant id, not durability
    ...
    // items whose meta is part of their key; all others key as :0
    private static final Set<Item> META_KEYED = new HashSet<>();
    private static boolean BUILT; // set on entry to ensureBuilt, re-entrancy guard
    ...
    public static int bucket(final ItemStack stack) {
        ...
        if (it instanceof ItemTool || it instanceof ItemShears || it instanceof ItemHoe || it instanceof ItemFishingRod) {
            return CAT_TOOLS;
        }
        if (it instanceof ItemFood) {
            return CAT_FOOD;
        }
        if (it instanceof ItemPotion) {
            return CAT_POTION;
        }
        if (it instanceof ItemEnderPearl || it instanceof ItemSnowball || it instanceof ItemEgg
                || it instanceof ItemFireball || it instanceof ItemExpBottle) {
            return CAT_THROW;
        }
    ...
    private static void ensureBuilt() {
        ...
        for (final Item item : Item.itemRegistry) {
            ...
            // an item whose getSubItems returned nothing collapses to one entry, so its meta is ignored
            ...
            for (final ItemStack stack : variants) {
                ...
                if (key == null || BY_KEY.containsKey(key)) {
                    continue;
                }
    ...
    /** Canonical "registry:meta" key for a stack; meta is zeroed unless it identifies a variant. */
    public static String keyOf(final ItemStack stack) {
        ...
        // during the build META_KEYED is incomplete, so fall back to getHasSubtypes
    ...
    public boolean categoryIncludes(final int catIdx, final ItemStack stack) {
        ...
        ensureBuilt(); // keyOf needs the palette built
        if (bucket(stack) != catIdx) {
            return false;
        }
    ...
    public void dropOnSlot(final int slotIdx, final String key) {
        if (slotIdx < 0 || slotIdx >= SLOTS || key == null) {
            return;
        }
    ...
    public void fromJson(final JsonElement json) {
        ...
        // a restore replaces the defaults rather than appending to them
