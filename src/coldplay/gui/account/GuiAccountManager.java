...
/**
 * Account and global game-server proxy editor. Network authentication runs off-thread;
 * applying sessions and persisting rotated refresh tokens runs on the client thread.
 */
public class GuiAccountManager extends GuiScreen {

    private static final int ROW_H = 22;
    private static final int PAD = 14;
    private static final int X_SIZE = 12;          // delete-button hit zone
    ...
    // Keep token/name storage separate so switching tabs cannot reveal a token in the username field.
    // Proxy slots must be last and contiguous: isProxyFieldFocused uses >= F_HOST; arrays use slot IDs.
    ...
    private int cursorCounter; // ticked in updateScreen, drives the blinking caret
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        if (small != null && proxyConfigured) {
            // A short badge avoids colliding with the label on narrow windows.
    ...
    private void drawList(final CustomFont font, final int mouseX, final int mouseY) {
        ...
        for (int i = 0; i < alts.size(); i++) {
            ...
            if (!this.rowVisible(rowY)) {
                continue; // fully outside the viewport; straddling rows render clipped by the scissor
            }
        ...
        // Draw the frame last so row hover cannot tint it.
        RenderUtil.outline(this.listX, this.listY, this.listX + this.listW, this.listBottom, 1, Theme.SEP);
    }
    ...
    private void drawRow(final CustomFont font, final Alt alt, final int rowY, final int mouseX, final int mouseY) {
        ...
        if (delHover) {
            RenderUtil.rect(delX, delY, X_SIZE, X_SIZE, Theme.HOVER_LIFT);
        ...
        if (font != null) {
            ...
            font.drawCenteredInRect("x", delX, delY, X_SIZE, X_SIZE,
                    delHover ? Theme.DANGER : Theme.TEXT_DIM);
        }
    }

    /** Wheel-only scroll indicator; the thumb is not draggable. */
    ...
    private int delX() {
        return this.listX + this.listW - X_SIZE - 4;
    }

    private static int delY(final int rowTop) {
        return rowTop + (ROW_H - X_SIZE) / 2;
    }

    private boolean hitsDelete(final int rowTop, final int mouseX, final int mouseY) {
        return RenderUtil.hovered(mouseX, mouseY, this.delX(), delY(rowTop), X_SIZE, X_SIZE);
    }
    ...
    public void handleMouseInput() throws IOException {
        // Use event coordinates so fast move-and-scroll input targets the current list.
    ...
    private void handleListClick(final int mouseX, final int mouseY) {
        if (mouseY < this.listY || mouseY > this.listBottom) {
            return; // stay in lockstep with the scissored render: only the visible strip is clickable
        }
        final List<Alt> alts = AltManager.getInstance().getAlts(this.crackedTab);
        for (int i = 0; i < alts.size(); i++) {
            final int rowY = this.rowTop(i);
    ...
    protected void keyTyped(final char typedChar, final int keyCode) throws IOException {
        // Let fields consume Esc before navigation, but exclude Enter so it still submits.
        if (this.focused != F_NONE
            ...
            if (!edit.isFocused()) {
                this.focused = F_NONE; // Esc unfocuses the field instead of closing the screen
            }
            return;
        }
    ...
    private void switchTab(final boolean cracked) {
        ...
        this.scrollOffset = 0; // the two histories have different lengths
        this.setStatus("", Theme.TEXT_DIM);
        // the tab's own input takes the caret, so the Enter key means Login rather than Save Proxy
        this.focused = cracked ? F_NAME : F_TOKEN;
    }

    private void loginOffline() {
        if (this.busy) {
            return; // don't race an in-flight premium login worker
        }
    ...
    private void startLogin() {
        ...
        this.fields[F_TOKEN] = "";
        // A Minecraft bearer token is a JWT (eyJ…); anything else (e.g. an M.C… value) is a refresh token.
        this.runLogin(trimmed, !looksLikeJwt(trimmed));
    }

    /** Recognizes Minecraft-services JWT access tokens; other input is treated as a refresh token. */
    private static boolean looksLikeJwt(final String token) {
        return token.startsWith("eyJ") && token.indexOf('.') != token.lastIndexOf('.');
    }

    /** Applies only the latest login attempt on the client thread and persists its rotated refresh token. */
