...
/**
 * Splash/menu backdrop. Inline GLSL is available before the resource pipeline initializes.
 * ShaderLoader handles GL/ARB compatibility; compilation failure selects an animated fallback.
 */
public final class BackgroundShader {
    ...
    private static boolean failed;    // Avoid retrying a failed compilation every frame.
    ...
    /**
     * Fills the current ortho viewport ({@code 0..guiWidth} × {@code 0..guiHeight} in GUI coords) with
     * the animated backdrop. {@code pixelWidth}/{@code pixelHeight} are the bound framebuffer's real
     * pixel size — the shader needs it because {@code gl_FragCoord} is in pixels, not GUI units.
     */
    public static void draw(int guiWidth, int guiHeight, int pixelWidth, int pixelHeight) {
        ...
        // wraps hourly for float precision; use elapsed time if the visible jump matters.
        timeBuffer.clear();
    ...
    /** Masks scrolled rows beneath title and action-bar regions. */
    ...
    /** No-op until callers load {@link Fonts}. */
    ...
    private static void compile() {
        ...
        try {
            ...
            program = prog;
            prog = 0; // ownership transferred to the static program field
        } catch (Exception e) {
            failed = true;
    ...
    /** Keeps the menu visibly animated when shader compilation fails. */
