...
/** Draggable category panel; the screen handles module clicks and shared docked borders. */
public class CategoryPanel {

    private static final int ARROW_X = 5;   // arrow inset from the panel's left edge
    private static final int NAME_X = 14;   // header text inset (past the arrow)
    private static final int RIGHT_PAD = 8; // header text right padding when measuring width
    ...
    /** Reveal progress: 0 is header-only, 1 fully expanded. */
    private final Animation reveal = new Animation(0.0, Theme.WIPE_SPEED);
    /** Animated height for borders; hit tests use the logical height. */
    ...
    /** Draw content separately so docked panels can share one border. */
    ...
    /** Draw after content so full-width row separators cannot cover the border. */
    public void renderChrome() {
        ClickGuiScreen.drawWindowFrame(x, y, width, visualHeight);
    }

    /** Draws a small 5x5 pixel triangle: ▶ when collapsed, ▼ when expanded (Monocraft has no arrow glyph). */
    ...
    /** Stores the current layout into the config (without writing to disk). */
    public void persist() {
    ...
    /** Instant (logical) height — hit tests and clamping use this, not the eased wipe height. */
    public int getHeight() {
        return totalHeight();
    }

    /** Height as drawn this frame (header + eased rows); the docked union silhouette hugs this. */
    public int getVisualHeight() {
        return visualHeight;
    }

    /** Returns -1 for a hidden/absent row, which closes its docked settings panel. */
