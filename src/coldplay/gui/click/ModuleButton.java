...
/**
 * Module row. The parent paints the translucent body once; rows overlay hover and enabled state.
 * Click handling belongs to the screen.
 */
public class ModuleButton {

    private static final int NAME_X = 6;       // name inset from the row's left edge
    private static final int QMARK_SIZE = 12;  // side of the square "?" button
    private static final int QMARK_MARGIN = 4; // gap from the row's right edge to the button
    private static final int NAME_GAP = 8;     // min gap between name and button (width calc)
    ...
    static int preferredWidth(Module module, CustomFont font) {
        // long key names can overlap the module name; include bind width if needed.
