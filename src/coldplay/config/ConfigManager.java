...
/** Coordinates config storage while {@link ConfigCodec} owns all JSON shapes. */
public class ConfigManager {
    ...
    /** Scaled screen size the panel anchors are expressed in; 0 until the first load or ClickGUI open. */
    private int panelLayoutWidth;
    ...
    /**
     * Re-anchor before GUI layout and saving so absolute scaled-GUI pixels retain their relative
     * placement across GUI-scale changes and window resizes.
     */
    ...
    /** @return false if the profile could not be written; the caller is expected to say so. */
    ...
    /** @return false if the profile is missing or carries nothing loadable, so nothing was applied. */
