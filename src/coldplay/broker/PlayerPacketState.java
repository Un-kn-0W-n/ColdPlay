...
/**
 * Last position and look dispatched on this connection, tracked after any packet holds. Attacks
 * prepared under {@link #atPose} are dropped at dispatch if that pose has since changed.
 */
public final class PlayerPacketState {
    ...
    /** Runs {@code action}; attack and swing packets it sends are checked against {@code pose} at dispatch. */
    ...
    public void prepare(Packet<?> packet) {
        ...
        if (accepted != null && (packet instanceof C02PacketUseEntity || packet instanceof C0APacketAnimation)) {
            synchronized (expected) {
                if (!expected.containsKey(packet)) {
                    expected.put(packet, accepted);
                }
            }
        }
    }

    /** Called at dispatch; false when a validated attack no longer matches the current pose. */
    public synchronized boolean accept(Packet<?> packet) {
        ...
        if (accepted != null && (!pose.isKnown() || accepted.epoch != pose.epoch
                || actionInWindow || !accepted.ray.test(pose))) {
            return false;
        }
