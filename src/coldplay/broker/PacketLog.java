...
/**
 * CSV log of every packet in and out, with outbound packets attributed to the module that produced them.
 * Called from the netty thread as well as the client thread.
 */
public final class PacketLog {
    ...
    /** Captures the producer now; the packet may be held and sent later from another stack. */
    public void prepare(Packet<?> packet) {
        if (!enabled) {
            return;
        }
        synchronized (origins) {
            if (!origins.containsKey(packet)) {
                origins.put(packet, captureProducer(packet, "prepare"));
            }
        }
    }
    ...
    /** Must not throw; an exception here disconnects the client. */
    ...
    /** Logs an automated packet that was refused before dispatch. */
    ...
    private void record(Packet<?> packet, String direction, int connection) {
        ...
        if (!"IN".equals(direction)) {
            ...
            if (prepared == null) {
                prepared = captureProducer(packet, "dispatch-fallback");
            }
            who = prepared.origin;
        ...
        try {
            ...
            if (packet instanceof C03PacketPlayer || packet instanceof C02PacketUseEntity
                ...
                for (StackTraceElement at : Thread.currentThread().getStackTrace()) {
                    ...
                    if ("Unknown".equals(producer)) {
                        producer = cls + "#" + at.getMethodName();
                    }
    ...
    /** First coldplay or minecraft frame on the current stack, as "Class#method". */
    ...
    private static String gameplayContext(Packet<?> packet) {
        ...
        if (mc == null || !mc.isCallingFromMinecraftThread() || mc.thePlayer == null) {
            return "; clientContext=unavailable";
        }
        ...
        if (packet instanceof C02PacketUseEntity && mc.theWorld != null) {
            ...
            if (target != null) {
                context += "; targetId=" + target.getEntityId()
    ...
    public void note(String kind, String fields, boolean whenDisabled) {
        if (enabled || whenDisabled) {
            append(System.currentTimeMillis(), System.nanoTime(), tick, frame,
                    "META", kind, "", fields, 0, "", "");
        }
    }
    ...
    private static void appendValue(StringBuilder out, Object value) {
        if (value instanceof ItemStack) {
            ...
            if (count < bytes.readableBytes()) {
                out.append("...truncated");
            }
        } else if (value instanceof IChatComponent) {
            // IChatComponent iterates over itself; serialise to JSON instead of recursing.
    ...
    /** Writes and clears the buffered rows; a failed write puts them back. */
