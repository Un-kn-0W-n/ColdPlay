...
public final class AntiFireBall extends Module {
    ...
    /** Runs in the shared safety stage, after broker reset and Auto Off, before ordinary modules. */
    ...
    public void onRender(EventRender event) {
        ...
        Vec3 interpolated = RenderUtil.interpolatedPosition(fireball, partial);
        double x = interpolated.xCoord;
        double y = interpolated.yCoord + box.getCenter().yCoord - fireball.posY;
        double z = interpolated.zCoord;
        Vec3 eyes = player.getPositionEyes(partial);
        float[] angles = RotationManager.angleTo(eyes.xCoord, eyes.yCoord, eyes.zCoord, x, y, z);
        RotationManager.getInstance().request(this, angles[0], angles[1], ResourcePriority.FALL_SAFETY, 180.0D);
    }

    /** Stop movement during the reach window, preserving manual and fall-safety sneak. */
    ...
    /** PRE at drain priority: RotationManager has written the look the C03 will carry, and the C03 is not queued yet. */
    @EventTarget(priority = EventPriority.DRAIN)
