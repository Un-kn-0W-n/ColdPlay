...
public class AutoHeal extends Module {
    ...
    public void onRender(EventRender event) {
        ...
        if (player != null) {
            RotationManager.getInstance().request(
                    this, player.rotationYaw, THROW_PITCH, PRIORITY, TURN_RATE);
        }
    }
    ...
    private void tickPotion(Minecraft mc, EntityPlayerSP player) {
        ...
        if (!rotations.owns(this)
                || Math.abs(rotations.getServerPitch() - THROW_PITCH) > PITCH_READY_DEG
                || !fireUse(mc, player)) {
            return;
        }
        reset();
