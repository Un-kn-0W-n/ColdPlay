...
public final class Breaker extends Module {
    ...
    private static final double TURN_RATE = 45.0; // deg/tick toward the dig point
    private static final int POST_BREAK_COOLDOWN = 2; // ticks to hold off after a block pops
    ...
    private Aim aim; // this tick's pick; written once per pre-update, null when idle or hurt-paused; read per-frame by onRender
    private Lock lock; // sticky pick, survives across ticks until stale or abandoned
    private BlockPos digging; // block the last dig packet was sent for
    private String lastMode; // mode seen last tick, to reset picks when the user flips it
    ...
    /** Runs right after KillAura publishes activity at NORMAL + 2, freeing dig and rotation before ordinary handlers. */
    ...
    /** Exposed bed: hit the nearer half directly. Covered bed: hold one adjacent cover block until it goes stale. */
    private Aim pickHypixel(EntityPlayerSP player, BedUtil.Bed bed) {
        ...
        // Face re-derived every tick: the eyes move while the covered block does not.
        return new Aim(lock.pos, faceTowardBlock(player.getPositionEyes(1.0F), lock.pos), null);
    }

    /** Digs only what a sight ray toward the bed hits; block, face, and hit point stay frozen while valid. */
    ...
    /** No line of sight to an occluded block — face it geometrically, by the largest eye delta. */
    ...
    /** Require the server look to hit the block before digging. */
    ...
    private float animatedProgress(Minecraft mc) {
        ...
        // Snap down on regress (new block, server reset) so the bar never eases backward.
    ...
    /** One tick's pick: the block to aim at this frame. hitVec is the exact Legit sight hit; null in Hypixel. */
    ...
    /** Sticky pick, held until stale. sight is the frozen Legit ray (sight.pos == pos); null in Hypixel. */
