...
public class KillAura extends Module {
    ...
    private static final long WALL_GRACE_MS = 500; // keep tracking through brief pillar peeks
    private static final double AIM_INSET = 0.05; // contract the hitbox so aim points land strictly inside it
    private static final double MARKER_HALF = 0.05;

    // Rotation graph: one sample per tick of the wire look and the ramp rates in a ring buffer, so the
    // oldest tick falls off the left. Per-tick deltas are derived at draw time from consecutive looks.
    ...
    /** Registered for the whole session, unlike the module, so the panel can be placed while KillAura is off. */
    ...
    public KillAura() {
        ...
        rotationSpeed.label("Yaw Speed"); // renaming the setting itself would orphan every saved profile
    ...
    public void onUpdate(EventUpdate event) {
        ...
        // Rotations stop being requested while a screen is up, so attacking on
        // the stale wire look would hit blind; pause the whole loop instead.
        if (paused(mc, player)) {
            return;
        }
        ...
        // PRE keeps the click before this tick's movement packet. No delay is added to a valid action.
        if (RotationManager.getInstance().owns(this) && now >= nextClickAt
            ...
            // From the old deadline, not now: the carried remainder mixes 2- and 3-tick gaps at one CPS.
    ...
    public void onRender(EventRender event) {
        ...
        Entity victim = target;
        // Same states that stop onUpdate from attacking: rotating here would be pure wire signature.
        ...
        if (aimTarget != victim) {
            ...
            // Vary the intended point once, in hitbox space, instead of adding angular noise every tick.
        ...
        double ticks = RotationManager.elapsedTicksSince(lastFrameNanos, now);
        lastFrameNanos = now;
        ...
        yawRate = ramp(yawRate, paced(rotationSpeed),
                ...
                paced(yawAccel) / 100.0, paced(yawDecel) / 100.0, ticks);
        pitchRate = ramp(pitchRate, paced(pitchSpeed),
                Math.abs(want[1] - fromPitch),
                paced(pitchAccel) / 100.0, paced(pitchDecel) / 100.0, ticks);
    ...
    // Read live so a slider edit lands next frame; only the pace is rolled per target.
    private double paced(RangeSetting setting) {
        return setting.getLo() + aimPace * (setting.getHi() - setting.getLo());
    }

    // Ease the rate toward a ceiling that drops near the target. The broker clamps each step to
    // the rate, so braking has to shorten the rate itself. Floored at one mouse count so it lands.
    private static double ramp(double rate, double cap, double error,
                               double accel, double decel, double ticks) {
        rate = Math.min(rate, cap); // a lowered slider has to bite on this frame
        double goal = decel > 0 ? Math.min(cap, error / (decel * 10.0)) : cap; // 100% brakes ten ticks out
        // Compounded per tick, so ten sparse frames and one whole tick reach the same rate.
        double gain = 1.0 - Math.pow(1.0 - (goal > rate ? accel : decel), ticks);
    ...
    public void onRender3D(EventRender3D event) {
        ...
        // Starting exactly at the camera collapses the tracer to one pixel.
    ...
    private void sampleGraph(EntityPlayerSP player) {
        RotationManager rm = RotationManager.getInstance();
        boolean aiming = aimTarget != null; // idle ticks record a zero rate, not the last ramp value
    ...
    /** One line strip, newest tick at the right edge; {@code delta} plots the per-tick change centred on zero. */
    ...
    private Vec3 aimPoint(AxisAlignedBB box, Vec3 eyes) {
        ...
        if (FIRST.equals(mode)) {
            ...
            // Looking away from the box: a probe along the look lands behind it
            // and closestPoint would pick the far face. Nearest surface instead.
