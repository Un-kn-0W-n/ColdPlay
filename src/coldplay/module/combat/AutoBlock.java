...
public class AutoBlock extends Module {
    ...
    public void onUpdate(EventUpdate event) {
        ...
        if (ActionGuard.getInstance().isReserved()) {
            ...
            state = State.IDLE;
            ticks = 0;
            return; // keep a just-completed attack armed for the next free tick
        }
        ...
        switch (state) {
            case IDLE:
                ...
                break;
            case BLOCKING:
                // a GUI freezes vanilla's onStoppedUsingItem, so isUsingItem() never clears and this
                // branch would re-reserve ActionGuard every tick, starving GUI modules
    ...
    /** Clears the vanilla tap-block state only; the packet hold is disarmed by its caller. */
