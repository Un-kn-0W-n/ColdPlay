...
/** Sends one inventory click per delay while the survival inventory is open, armor first, then hotbar, then cleaner. */
public class InvManager extends Module {
    ...
    public void onUpdate(EventUpdate event) {
        ...
        if ((click[0] >= 36 || click[2] == 2) && !SlotGuard.getInstance().isFree()) {
            return;
        }
        ...
        PacketLog.getInstance().tagged("InvManager", () -> {
            if (!InvUtil.click(player, container, click[0], click[1], click[2])) {
                return;
            }
            worked = true;
            lastSlot = click[0];
            int[] after = next(container); // click already predicted locally
    ...
    // An occupied armor slot gets shift-clicked out before the better piece goes in.
    ...
    private int bestArmor(Container c, int type) {
        ...
        for (int slot = 9; slot < 45; slot++) {
            ...
            if (stack == null || !(stack.getItem() instanceof ItemArmor) || ((ItemArmor) stack.getItem()).armorType != type) {
                continue;
            }
    ...
    // Greedy; earlier hotbar slots claim first.
    ...
    private static double scoreArmor(ItemStack stack) {
        ...
        int material = 0;
        for (int t = 0; t < 4; t++) {
            material += armor.getArmorMaterial().getDamageReductionAmount(t);
        }
