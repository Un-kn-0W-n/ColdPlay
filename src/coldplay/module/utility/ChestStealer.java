...
/** Shift-clicks the nearest stack out of an open chest; one click per stack, closes via InvManager's Auto Close. */
public class ChestStealer extends Module {
    ...
    public void onUpdate(EventUpdate event) {
        ...
        if (target == null) {
            // empty or nothing fits; pending replies could still refute the local prediction
            if (autoClose.getAsBoolean() && !transactions.hasPending()) player.closeScreen();
            return;
        }
        if (!ActionGuard.getInstance().tryReserve(this)) return;
        int slot = target.slotNumber;
        PacketLog.getInstance().tagged("ChestStealer", () -> {
            if (!InvUtil.click(player, container, slot, 0, 1)) return;
            Slot next = nearest(container, chestSlots, slot); // click already predicted the transfer locally
    ...
    // NPC menus reuse GuiChest; vanilla chest names all contain "chest". Block-hit tracking if a server names a shop "Chest".
    ...
    // Nearest takeable stack to the last click: shortest cursor trip, one click per stack.
    static Slot nearest(Container container, int chestSlots, int fromSlot) {
        ...
        for (int i = 0; i < chestSlots; i++) {
            ...
            if (!slot.getHasStack() || !fits(container, chestSlots, slot.getStack())
                    || !InventoryTransactions.getInstance().canClick(container, i, 0, 1)) continue;
    ...
    // Read-only Container.mergeItemStack over the player slots (that one is protected and mutates).
    static boolean fits(Container c, int chestSlots, ItemStack stack) {
        for (int i = chestSlots; i < c.inventorySlots.size(); i++) {
            ...
            if (stack.isStackable() && in.getItem() == stack.getItem()
                    ...
                    && in.stackSize < stack.getMaxStackSize()) return true;
        }
        return false;
    }
}
