...
/**
 * Samples camera and rewritten Motion PRE rotations before packet dispatch.
 * A successful CSV export clears the buffer; failed writes preserve the recording.
 */
public class HumanRecorder extends Module {
    ...
    // drop-oldest cap, ~1 hour at 20 tps; raise if longer sessions matter
    private static final int MAX_SAMPLES = 72_000;

    /** Each sample: cameraYaw, cameraPitch, sentYaw, sentPitch. */
    ...
    // DRAIN observes the rewrite; buffering and correction acknowledgements occur after this sample.
    ...
    /** Synchronous on the client thread; MAX_SAMPLES bounds the stall. */
    private void exportCsv() {
        ...
        File directory = new File(new File(Minecraft.getMinecraft().mcDataDir, "coldplay"),
                "recordings");
        // Re-check exists(): mkdirs() returns false when the directory already got created.
