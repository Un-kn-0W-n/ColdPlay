...
/** Disabling stops capture but keeps the buffer, so it can still be exported. */
public class PacketLogger extends Module {
    ...
    public void onUpdate(EventUpdate event) {
        ...
        // Settings are edited in the GUI. Do not serialize the profile on the combat hot path.
        if (inGui || wasInGui) recordConfigSnapshot(false);
        wasInGui = inGui;
    }
    ...
    private void recordConfigSnapshot(boolean force) {
        try {
            ...
            if (force || !config.equals(lastConfig)) PacketLog.getInstance().note("EffectiveConfig", config, force);
            lastConfig = config;
    ...
    private void recordSessionContext(boolean force) {
        ...
        if (force || !session.equals(lastSession)) PacketLog.getInstance().note("SessionContext", session, force);
        lastSession = session;
    }
    ...
    private void exportCsv() {
        ...
        recordSessionContext(true);
        recordConfigSnapshot(true);
        // File I/O must not stall the client tick or packet dispatch.
        ...
        // Let an export already requested by the user finish during normal client shutdown.
    ...
    private static void message(String message, boolean success) {
        Minecraft.getMinecraft().addScheduledTask(() -> {
            if (success) ChatUtil.success(message); else ChatUtil.error(message);
        });
    }
}
