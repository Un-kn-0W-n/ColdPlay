...
public final class BedProtection extends Module {
    ...
    private static final double TURN_RATE = 180.0D; // deg/tick toward the target cell

    private final Random rand = new Random();
    private Placement pending;   // cell being walled; re-selected each tick, read per-frame by onRender
    private boolean forcedSneak; // we forced sneak for a bed-support place and must clear it
    ...
    public void onUpdate(EventUpdate event) {
        ...
        // Keep the prior hit point while the same cell/support/face stays the pick — stable aim.
        ...
        if (held == null || !(held.getItem() instanceof ItemBlock)) {
            ...
            if (slot == -1 || !SlotGuard.getInstance().request(this, slot, PRIORITY)) {
                release(mc, player);
                return; // no block to place with; keep aiming, retry next tick
            }
        ...
        if (mop == null) {
            return; // sent look isn't on the face yet — keep turning
        }

        // A bare right-click on a bed sleeps instead of placing; the server honours the sneak bypass
        // only after it has seen the C0B (vanilla syncs the flag), so start now, place a later tick.
        ...
        if (!ActionGuard.getInstance().tryReserveAfterCleanTick(this)) {
            release(mc, player);
            return; // the player's own interaction (or another module) owns this window
        }
    ...
    /** Drop the held-slot claim and any forced sneak; keeps {@link #pending} so aiming continues. */
    ...
    /** Prefer non-bed supports to avoid the extra sneak window needed before clicking a bed. */
    ...
    /**
     * Order by Chebyshev shell then eye distance. Bed-local coordinates keep grid choices
     * stable across bed orientations; include the lid directly above the bed.
     */
                ...
                for (int z = minZ - layer; z <= maxZ + layer; z++) {
                    for (int y = bedY; y <= bedY + layer; y++) {
                        ...
                        if (Math.max(outX, Math.max(outY, outZ)) != layer) {
                            continue; // not this shell (already emitted by an inner layer, or the bed itself)
                        }
                        int horiz = Math.max(outX, outZ);
                        if (horiz == 0) {
                            if (outY != 1) {
                                continue; // over the bed: keep only the single lid at bed+1
                            }
                        } else if (horiz != layer) {
                            continue; // Omit roofs over inner columns.
                        } else {
                            int dx = x - foot.getX();
                            int dz = z - foot.getZ();
    ...
    /** Same support/reach/visibility evaluator as real placement, with stable face centres. */
