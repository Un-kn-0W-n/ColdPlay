...
/** Warns in chat as enemies cross closer distance bands around the player's bed; only during a BedWars match. */
public final class BedNotification extends Module {
    ...
    /** Hypixel's in-game sidebar lists every team as "R Red: ✓" (or ✘ once its bed is gone); no lobby draws those. */
    ...
    // glyph sniff, Hypixel-only like the rest of the bed tooling; swap for a scoreboard title check if another server matters.
    ...
    /** Returns the closest latched band, retaining it through the outer anti-jitter margin. */
