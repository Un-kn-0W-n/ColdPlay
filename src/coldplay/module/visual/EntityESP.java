...
/**
 * One entity snapshot per frame feeds every mode. Boxes, arrows and name tags project in the
 * world pass and draw on the HUD pass; Outline and Chams hand the selection to their registries.
 */
public class EntityESP extends Module {
    ...
    /** Lines are drawn in world space, so bobbing would pull them off the crosshair. */
    ...
    private void drawArrows(float width, float height, boolean mirror) {
        ...
        for (ScreenArrow arrow : arrows2D) {
            ...
            if (avoidGui) {
                ...
                effRadius = Math.max(radius, Math.min(tx, ty) + size); // +size so the whole arrowhead clears
            }
