...
/** Static blocks are scanned on tick and drawn in 3D; dropped items are projected per frame and drawn on the HUD. */
public class BlockESP extends Module {
    ...
    // Same-type items within this half-edge share one summed tag (servers that block stack merging litter "1"s otherwise).
    ...
    // pos -> placed-at ms; insertion order so eviction drops the oldest
    ...
    // Raw framebuffer px, filled in the world pass and drained by the HUD pass.
    ...
    public void onUpdate(EventUpdate event) {
        ...
        // Render doesn't re-check the owner setting, so drain here when it's off.
    ...
    /** Prune on tick so the render pass never touches chunks. */
    ...
    /** Loaded tile entities beat a block-volume scan. */
    ...
    /** Beds have no tile entity in 1.8.9, so throttled block scan. */
    // fixed 20-tick rescan; switch to event-driven tracking if staleness ever matters.
    ...
    public void onRenderMirror(EventRenderMirror event) {
        if (event.getStage() == EventRenderMirror.Stage.WORLD) renderView(event.getPartialTicks());
        else renderOverlay(event.getScale());
    }
    ...
    /** Project items now while the world camera matrices are current; the HUD pass draws them later. */
    private void collectItems(float partialTicks) {
        ...
        for (Entity entity : world.loadedEntityList) {
            ...
            if (!captured) {
                // glGet* stalls the pipeline; only pay for it once there's something to project
                captured = true;
        ...
        for (TagCluster cluster : itemTags2D) {
            // 0.25 entity height plus the hover-bob RenderEntityItem floats the model with
    ...
    /** One buffer, one draw: every enabled owner's boxes as fills (GL_QUADS) or edges (GL_LINES). */
    ...
    // greedy first-anchor clustering; piles wider than 3 blocks split into several tags,
    // which reads fine. Union-find only if that ever looks wrong.
