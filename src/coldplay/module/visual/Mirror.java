...
public final class Mirror extends Module {
    ...
    /** EventRender stops after a disconnect, so the world check also runs on tick. */
    @EventTarget
    ...
    /** Stretches the current texture while dragging; the capture resizes on release. */
    ...
    private static String unavailable() {
        if (Config.isShaders()) {
            return "Unavailable with shader packs";
        }
        if (!OpenGlHelper.isFramebufferEnabled()) {
            return "Requires FBO; Fast Render / AA off";
        }
        return null;
    }
    ...
    public void onRender2D(EventRender2D event) {
        Minecraft mc = Minecraft.getMinecraft();
        if (mc.thePlayer == null || (mc.gameSettings.hideGUI && !hud.isEditing())) {
            return;
        }
        ...
        if (message == null && !hasFrame) {
            message = failed ? "Capture failed - toggle to retry" : "Mirror";
        }
        if (message != null) {
            ...
            // the framebuffer alpha is not panel transparency; alpha test would punch holes
