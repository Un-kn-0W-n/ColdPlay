...
public class Trajectory extends Module {
    ...
    @SuppressWarnings("unused") // held by the settings list; the entries above are the read points
    ...
    /** Entity filter for the flight sim; a field so the per-step call allocates nothing. */
    ...
    public void onRender3D(EventRender3D event) {
        ...
        if (speed <= 0.0) {
            return; // not holding an enabled projectile item
        }
    ...
    /** Use live bow charge while drawing and full charge while merely holding the bow. */
    ...
    /** Shared Arrow/Throwable profiles preserve vanilla spawn, motion, drag and collision rules. */
    private Flight simulate(final EntityPlayerSP player, WorldClient world, double speed,
        ...
        for (int tick = 0; tick < MAX_SIM_TICKS; tick++) {
            ...
            if (state.getPosition().yCoord < 0.0) {
                break; // into the void: nothing left to hit, so stop tracing and allocating
            }
        }
        return flight;
    }
