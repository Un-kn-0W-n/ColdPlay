...
public class BridgeAssist extends Module {
    ...
    /** Keep protecting an emptied block slot until the player reaches safe ground. */
    private EntityPlayerSP bridgingPlayer;
    /** Sneak ticks still owed once the edge is gone, so a pulse never matches the edge window exactly. */
    ...
    public void onStrafe(EventStrafe event) {
        ...
        // An owed sneak keeps the session alive, or the next tick would end the bridge mid-pulse.
        ...
        if ((atEdge || sneakHoldTicks > 0) && player.movementInput.moveForward <= 0.0F) {
            ...
            sneakHoldTicks = 0; // stepping forward ends the pulse the same way it prevents one
        }
    }

    /** Deliberately accepts any ItemBlock, including slabs and stairs, rather than only full cubes. */
