...
public final class BedUtil {
    ...
    public static Bed findNearestBed(int radius, BlockPos excludeFoot) {
        ...
        for (BlockPos.MutableBlockPos mutablePos : BlockPos.getAllInBoxMutable(from, to)) {
            ...
            // The mutable iterator reuses one object, so persist an immutable coordinate before storing it.
        ...
        for (int cx = playerChunkX - chunkRadius; cx <= playerChunkX + chunkRadius; cx++) {
            for (int cz = playerChunkZ - chunkRadius; cz <= playerChunkZ + chunkRadius; cz++) {
                ...
                for (net.minecraft.world.chunk.storage.ExtendedBlockStorage storage : chunk.getBlockStorageArray()) {
                    ...
                    // State IDs are blockId << 4 | meta; inspect them directly to avoid a registry lookup per cell.
    ...
    /**
     * One candidate pipeline for both breaker modes. Hypixel mode asks for the best adjacent position;
     * Legit mode asks for the first valid full ray hit. Validation, reach and visibility scoring live here.
     */
