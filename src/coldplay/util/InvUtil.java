...
public final class InvUtil {
    ...
    // Keep travel-dependent delay even for narrow or collapsed ranges.
    ...
    public static boolean isPlayerMoving(final EntityPlayerSP p) {
        ...
        return p.isSprinting()
                ...
                // Include residual motion, using vanilla's position-send threshold.
                || dx * dx + dz * dz > 9.0E-4D;
    }
    ...
    public static int findHotbarSlot(final EntityPlayerSP player, final Predicate<ItemStack> predicate) {
        ...
        for (int slot = 0; slot < HOTBAR_SIZE; slot++) {
            if (predicate.apply(player.inventory.mainInventory[slot])) return slot;
        }
        return -1;
    }
    ...
    public static boolean click(EntityPlayerSP player, Container container, int slot, int button, int mode) {
        ...
        if (player == null || mc.playerController == null || container == null || player.openContainer != container) return false;
        if (mode == 2 && (button < 0 || button >= HOTBAR_SIZE)) return false;
        if (mode == 4 && (slot < 9 || slot > 35 || player.inventory.getItemStack() != null)) return false;
    ...
    // Sample once for the upcoming move; applying it to the previous move changes the travel relationship.
