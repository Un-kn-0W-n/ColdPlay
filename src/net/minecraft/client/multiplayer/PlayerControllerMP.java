...
public class PlayerControllerMP
{
    ...
    private World lastDigEventWorld;
    private int lastDigEventTick = Integer.MIN_VALUE;
    private BlockPos lastDigEventPos;
    ...
    {
        // ColdPlay >>> EventDig / dig funnel: a listener may switch the held slot (AutoTool); flush
        // its C09 here — clickBlock never syncs on its own — so it precedes this dig's ABORT/START
        // C07s and the currentItemHittingBlock capture below. onPlayerDamageBlock delegates to
        // clickBlockBody directly since it already posted+synced this tick — one funnel entry per dig.
        this.postDigOnce(loc);
        this.syncCurrentPlayItem();
        return this.clickBlockBody(loc, face);
    }

    private boolean clickBlockBody(BlockPos loc, EnumFacing face)
    {
        // ColdPlay <<<
        if (this.currentGameType.isAdventure())
        {
    ...
    {
        // Breaker releases this guard before its own reset.
        if (coldplay.broker.DigGuard.isOwned()) return;
        // ColdPlay <<<
        if (this.isHittingBlock)
        {
    ...
    {
        // ColdPlay >>> EventDig / per-tick dig: posted before vanilla's own sync below, so a
        // listener's slot change flushes as this tick's C09 first.
        this.postDigOnce(posBlock);
        // ColdPlay <<<
        this.syncCurrentPlayItem();
        ...
        {
            // ColdPlay: clickBlockBody, not clickBlock — this call already posted EventDig + synced above.
            return this.clickBlockBody(posBlock, directionFacing);
        }
    }

    private void postDigOnce(BlockPos pos) {
        int tick = this.mc.thePlayer == null ? Integer.MIN_VALUE : this.mc.thePlayer.ticksExisted;
        if (this.mc.theWorld == this.lastDigEventWorld && tick == this.lastDigEventTick
                && pos.equals(this.lastDigEventPos)) {
            return;
        }
        this.lastDigEventWorld = this.mc.theWorld;
        this.lastDigEventTick = tick;
        this.lastDigEventPos = pos;
        coldplay.ColdPlay.post(new coldplay.event.EventDig(pos));
    }
    ...
    public void attackEntity(EntityPlayer playerIn, Entity targetEntity)
    {
        ...
        if (this.currentGameType != WorldSettings.GameType.SPECTATOR)
        {
            ...
            coldplay.ColdPlay.post(new coldplay.event.EventAttackPerformed(targetEntity, wasSprinting));
        }
    }
    ...
    /** Automation uses the same prediction/send path, with rejection recovery and no normal-path wait. */
    ...
    private ItemStack windowClick(int windowId, int slotId, int mouseButtonClicked, int mode, EntityPlayer playerIn, boolean automated)
    {
        short short1 = playerIn.openContainer.getNextTransactionID(playerIn.inventory);
        if (automated) coldplay.broker.InventoryTransactions.getInstance().sent(
                playerIn.openContainer, short1, slotId, mouseButtonClicked, mode);
    ...
    // ColdPlay >>> Breaker's HUD bar / block highlight need the REAL break progress. Vanilla keeps
    // curBlockDamageMP private and only leaks it quantized to tenths and one step behind
    // (sendBlockBreakProgress -> RenderGlobal.damagedBlocks); this getter is the root fix.
