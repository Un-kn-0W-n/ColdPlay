...
public class GuiConnecting extends GuiScreen
{
    ...
    private String proxyDisplay; // ColdPlay: non-null while tunneled through the SOCKS5 proxy
    ...
    private void connect(final String ip, final int port)
    {
        ...
        // ColdPlay: note the active proxy for the screen and error text
        final coldplay.account.ProxyConfig activeProxy = coldplay.account.ProxyManager.getInstance().getActiveProxy();
        this.proxyDisplay = activeProxy == null ? null : activeProxy.getDisplayAddress();
        (new Thread("Server Connector #" + CONNECTION_ID.incrementAndGet())
        {
            public void run()
            {
                ...
                try
                {
                    ...
                    final boolean useNative = GuiConnecting.this.mc.gameSettings.isUsingNativeTransport();
                    // ColdPlay: ProxyManager decides proxy vs direct connect
                    GuiConnecting.this.networkManager = coldplay.account.ProxyManager.getInstance()
                            .connect(ip, port, useNative);
                ...
                catch (Exception exception)
                {
                    ...
                    // ColdPlay: when tunneled, the failed TCP target is the proxy
                    if (GuiConnecting.this.proxyDisplay != null)
                    {
                        s = "Failed to connect via SOCKS5 proxy " + GuiConnecting.this.proxyDisplay + ": " + s;
                    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        // ColdPlay: show the active proxy
        if (this.proxyDisplay != null)
        {
            this.drawCenteredString(this.fontRendererObj, "via SOCKS5 proxy " + this.proxyDisplay, this.width / 2, this.height / 2 - 35, 0xA0A0A0);
        }
