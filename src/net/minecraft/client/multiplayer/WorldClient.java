...
public class WorldClient extends World
{
    ...
    public boolean invalidateRegionAndSetBlock(BlockPos pos, IBlockState state)
    {
        ...
        // ColdPlay >>> EventBlockPlace: this is the sole funnel for S22/S23 server block changes
        // (chunk data takes another path), so replaceable -> non-air here == a confirmed placement.
        boolean placed = this.getBlockState(pos).getBlock().getMaterial().isReplaceable()
                && state.getBlock() != net.minecraft.init.Blocks.air;
        boolean changed = super.setBlockState(pos, state, 3);
        if (changed && placed)
        {
            coldplay.ColdPlay.post(new coldplay.event.EventBlockPlace(pos));
        }
        return changed;
    }
