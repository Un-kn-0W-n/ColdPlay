...
public class GuiIngame extends Gui {
    ...
    public void renderGameOverlay(final float partialTicks) {
        ...
        GlStateManager.enableAlpha();
        // ColdPlay >>> EventRender2D (HUD overlay, drawn on top of vanilla)
    ...
    protected void renderTooltip(final ScaledResolution sr, final float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
            ...
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
            final int left = sr.getScaledWidth() / 2 - 91;
            final int top = sr.getScaledHeight() - 22;
            // ColdPlay >>> LabelClient hotbar: rounded dark panel + white selected-slot pill instead of the
            // widgets.png texture; depth off so the rects never occlude the item icons drawn after them
            GlStateManager.disableDepth();
            coldplay.util.RenderUtil.drawRoundedRect(left - 1, top - 1, 184, 24, 6, 0xF01C1C1C);
            coldplay.util.RenderUtil.drawRoundedRect(left + entityplayer.inventory.currentItem * 20, top, 22, 22, 4, 0x40FFFFFF);
            GlStateManager.enableDepth();
            GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
            // ColdPlay <<<
