...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks)
    {
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, new Object[] {entity, this, Double.valueOf(x), Double.valueOf(y), Double.valueOf(z)}))
        {
            ...
            try
            {
                ...
                // ColdPlay >>> draw the spoofed look-pitch on our own player model only (the camera was already
                // built from the real rotationPitch in orientCamera, so this leaves the first-person view silent).
                if (entity == Minecraft.getMinecraft().thePlayer
                        && !Float.isNaN(((EntityPlayerSP) entity).coldplayRenderPitch)) {
                    f7 = ((EntityPlayerSP) entity).coldplayRenderPitch;
                }
                // ColdPlay <<<
    ...
    {
        int i = 16777215;
        int outlineColor = coldplay.broker.OutlineRegistry.getInstance().colorOf(entityLivingBaseIn);
        ...
        if (coldplay.broker.ChamsRegistry.getInstance().isActive())
        {
            // Keep texture alpha in the Chams mask: chicken feet and other cutouts are not solid quads.
            GlStateManager.enableTexture2D();
            GL11.glTexEnvi(GL11.GL_TEXTURE_ENV, GL11.GL_TEXTURE_ENV_MODE, OpenGlHelper.GL_COMBINE);
            GL11.glTexEnvi(GL11.GL_TEXTURE_ENV, OpenGlHelper.GL_COMBINE_RGB, GL11.GL_REPLACE);
            GL11.glTexEnvi(GL11.GL_TEXTURE_ENV, OpenGlHelper.GL_SOURCE0_RGB, OpenGlHelper.GL_PRIMARY_COLOR);
