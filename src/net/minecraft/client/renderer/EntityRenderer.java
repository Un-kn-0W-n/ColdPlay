...
public class EntityRenderer implements IResourceManagerReloadListener {
	...
	private Framebuffer mirrorTarget;
	private int mirrorScale;
	private float mirrorYaw, mirrorPitch;
	private final MirrorRenderState mirrorState = new MirrorRenderState();

	public boolean isRenderingMirror() { return mirrorTarget != null; }
	public float getMirrorYaw() { return mirrorYaw; }
	public float getMirrorPitch() { return mirrorPitch; }
	public Entity getWorldViewEntity() { return isRenderingMirror() ? mc.thePlayer : mc.getRenderViewEntity(); }
	public Framebuffer getWorldFramebuffer() { return isRenderingMirror() ? mirrorTarget : mc.getFramebuffer(); }
	private int worldWidth() { return isRenderingMirror() ? mirrorTarget.framebufferWidth : mc.displayWidth; }
	private int worldHeight() { return isRenderingMirror() ? mirrorTarget.framebufferHeight : mc.displayHeight; }

	public static float rearYaw(float previous, float current, float partialTicks) {
		return previous + (current - previous) * partialTicks + 180.0F;
	}

	public void renderMirror(Framebuffer target, float partialTicks, int scale) {
		final ActiveRenderInfo.State activeRenderInfo = new ActiveRenderInfo.State();
		mirrorState.capture();
		final boolean profiling = mc.mcProfiler.profilingEnabled;
		final boolean worldProfiling = mc.theWorld.theProfiler.profilingEnabled;
		final boolean fancy = mc.gameSettings.fancyGraphics;
		final boolean oldCloudFog = cloudFog;
		final float oldFogRed = fogColorRed, oldFogGreen = fogColorGreen, oldFogBlue = fogColorBlue;
		final float oldViewYaw = mc.getRenderManager().playerViewY, oldViewPitch = mc.getRenderManager().playerViewX;
		final float oldTileYaw = TileEntityRendererDispatcher.instance.entityYaw;
		final float oldTilePitch = TileEntityRendererDispatcher.instance.entityPitch;
		mc.mcProfiler.startSection("mirror");
		// Native passes rename/pop profiler sections internally. Keep one bounded outer sample even on failure.
		mc.mcProfiler.profilingEnabled = false;
		mc.theWorld.theProfiler.profilingEnabled = false;
		mirrorTarget = target;
		mirrorScale = scale;
		mirrorYaw = rearYaw(mc.thePlayer.prevRotationYaw, mc.thePlayer.rotationYaw, partialTicks);
		mirrorPitch = mc.thePlayer.prevRotationPitch + (mc.thePlayer.rotationPitch - mc.thePlayer.prevRotationPitch) * partialTicks;
		try {
			...
			this.renderWorldPass(2, partialTicks, 0L);
			mc.renderGlobal.renderEntityOutlineFramebuffer();
			GlStateManager.matrixMode(GL11.GL_PROJECTION);
			GlStateManager.loadIdentity();
			GlStateManager.ortho(0, target.framebufferWidth / (double) scale, target.framebufferHeight / (double) scale, 0, 1000, 3000);
			...
			postMirror(EventRenderMirror.Stage.OVERLAY, partialTicks);
		} finally {
			mirrorTarget = null;
			mc.gameSettings.fancyGraphics = fancy;
			cloudFog = oldCloudFog;
			fogColorRed = oldFogRed;
			fogColorGreen = oldFogGreen;
			fogColorBlue = oldFogBlue;
			mc.getRenderManager().playerViewY = oldViewYaw;
			mc.getRenderManager().playerViewX = oldViewPitch;
			TileEntityRendererDispatcher.instance.entityYaw = oldTileYaw;
			TileEntityRendererDispatcher.instance.entityPitch = oldTilePitch;
			mc.renderGlobal.displayListEntitiesDirty = true;
			mc.mcProfiler.profilingEnabled = profiling;
			mc.theWorld.theProfiler.profilingEnabled = worldProfiling;
			mc.mcProfiler.endSection();
			mc.getFramebuffer().bindFramebuffer(false);
			mirrorState.restore();
			activeRenderInfo.restore();
		}
	}

	private void postMirror(EventRenderMirror.Stage stage, float partialTicks) {
		coldplay.ColdPlay.post(new EventRenderMirror(stage, partialTicks, worldWidth(), worldHeight(), mirrorScale, mirrorYaw));
	}

	/** Restore through GlStateManager so its cache agrees with GL; glPopAttrib alone leaves stale cache entries. */
	private static final class MirrorRenderState {
		private final IntBuffer viewport = GLAllocation.createDirectIntBuffer(4);
		private final FloatBuffer color = GLAllocation.createDirectFloatBuffer(4);
		private final IntBuffer mask = GLAllocation.createDirectIntBuffer(4);
		private final int[] bindings = new int[3];
		private final boolean[] textures = new boolean[3];
		private final int[] textureDepths = new int[3];
		private int activeTexture, matrixMode, projectionDepth, modelDepth, depthFunc, shade;
		private int srcRgb, dstRgb, srcAlpha, dstAlpha, alphaFunc;
		private float alphaRef;
		private boolean depth, depthMask, blend, alpha, cull, lighting, fog, scissor, normalize, rescale, material;
		...
		void capture() {
			...
			projectionDepth = push(GL11.GL_PROJECTION, GL11.GL_PROJECTION_STACK_DEPTH);
			modelDepth = push(GL11.GL_MODELVIEW, GL11.GL_MODELVIEW_STACK_DEPTH);
			for (int i = 0; i < 3; i++) {
				GlStateManager.setActiveTexture(OpenGlHelper.defaultTexUnit + i);
				bindings[i] = GL11.glGetInteger(GL11.GL_TEXTURE_BINDING_2D);
				textures[i] = GL11.glIsEnabled(GL11.GL_TEXTURE_2D);
				textureDepths[i] = push(GL11.GL_TEXTURE, GL11.GL_TEXTURE_STACK_DEPTH);
			}
			...
			srcRgb = GL11.glGetInteger(org.lwjgl.opengl.GL14.GL_BLEND_SRC_RGB);
			dstRgb = GL11.glGetInteger(org.lwjgl.opengl.GL14.GL_BLEND_DST_RGB);
			srcAlpha = GL11.glGetInteger(org.lwjgl.opengl.GL14.GL_BLEND_SRC_ALPHA);
			dstAlpha = GL11.glGetInteger(org.lwjgl.opengl.GL14.GL_BLEND_DST_ALPHA);
			...
			material = GL11.glIsEnabled(GL11.GL_COLOR_MATERIAL);
			shade = GL11.glGetInteger(GL11.GL_SHADE_MODEL);
		}

		private static int push(int mode, int depthName) {
			GlStateManager.matrixMode(mode);
			int depth = GL11.glGetInteger(depthName);
			GlStateManager.pushMatrix();
			return depth;
		}

		private static void unwind(int mode, int depthName, int depth) {
			GlStateManager.matrixMode(mode);
			while (GL11.glGetInteger(depthName) > depth) GlStateManager.popMatrix();
		}

		void restore() {
			unwind(GL11.GL_PROJECTION, GL11.GL_PROJECTION_STACK_DEPTH, projectionDepth);
			unwind(GL11.GL_MODELVIEW, GL11.GL_MODELVIEW_STACK_DEPTH, modelDepth);
			for (int i = 0; i < 3; i++) {
				GlStateManager.setActiveTexture(OpenGlHelper.defaultTexUnit + i);
				unwind(GL11.GL_TEXTURE, GL11.GL_TEXTURE_STACK_DEPTH, textureDepths[i]);
				GlStateManager.bindTexture(bindings[i]);
				if (textures[i]) GlStateManager.enableTexture2D(); else GlStateManager.disableTexture2D();
			}
			GlStateManager.setActiveTexture(activeTexture);
			GlStateManager.matrixMode(matrixMode);
			...
			if (material) GlStateManager.enableColorMaterial(); else GlStateManager.disableColorMaterial();
		}
	}
	...
	public void getMouseOver(final float partialTicks) {
		...
		if (entity != null && this.mc.theWorld != null) {
			...
			boolean enforceSurvivalEntityReach = false;

			if (this.mc.playerController.extendedReach()) {
				entityReach = 6.0D;
			} else if (blockReach > 3.0D) {
				enforceSurvivalEntityReach = true;
			}

			final Vec3 eyes = entity.getPositionEyes(partialTicks);
			final Vec3 look = entity.getLook(partialTicks);
			RayPicker.Result result = RayPicker.pick(
					this.mc.theWorld, entity, eyes, look,
					blockReach, entityReach, 1.0D,
					null, false, false, true);
			this.pointedEntity = result.getEntity();

			if (this.pointedEntity != null
					&& enforceSurvivalEntityReach
					&& eyes.distanceTo(result.getEntityHitVec())
					> 3.0D + coldplay.broker.CombatManager.getInstance().getReachBonus()) {
				Vec3 rejectedHit = result.getEntityHitVec();
				this.pointedEntity = null;
				this.mc.objectMouseOver = RayPicker.miss(rejectedHit);
			} else {
				this.mc.objectMouseOver = result.getSelectedHit();
			}
	...
	private float getFOVModifier(final float partialTicks, final boolean useFOVSetting) {
		if (this.debugView) return 90.0F;
			...
			final Entity entity = this.getWorldViewEntity();
			float f = 70.0F;
			if (useFOVSetting) {
	...
	private void hurtCameraEffect(final float partialTicks) {
		if (this.getWorldViewEntity() instanceof EntityLivingBase) {
			final EntityLivingBase entitylivingbase = (EntityLivingBase) this.getWorldViewEntity();
	...
	private void setupViewBobbing(final float partialTicks) {
		if (this.getWorldViewEntity() instanceof EntityPlayer) {
			final EntityPlayer entityplayer = (EntityPlayer) this.getWorldViewEntity();
	...
	private void orientCamera(final float partialTicks) {
		...
		if (isRenderingMirror()) {
			GlStateManager.rotate(mirrorPitch, 1, 0, 0);
			GlStateManager.rotate(mirrorYaw + 180.0F, 0, 1, 0);
			GlStateManager.translate(0, -f, 0);
			cloudFog = mc.renderGlobal.hasCloudFog(entity.prevPosX + (entity.posX - entity.prevPosX) * partialTicks,
					entity.prevPosY + (entity.posY - entity.prevPosY) * partialTicks + f,
					entity.prevPosZ + (entity.posZ - entity.prevPosZ) * partialTicks, partialTicks);
			return;
		}
	...
	public void setupCameraTransform(final float partialTicks, final int pass) {
		...
		GlStateManager.loadIdentity();
		final float f = 0.07F;
		if (!isRenderingMirror() && this.mc.gameSettings.anaglyph) GlStateManager.translate((-(pass * 2 - 1)) * f, 0.0F, 0.0F);
		...
		Project.gluPerspective(this.getFOVModifier(partialTicks, true), (float) worldWidth() / (float) worldHeight(), 0.05F, this.clipDistance);
		GlStateManager.matrixMode(5888);
		GlStateManager.loadIdentity();
		if (!isRenderingMirror() && this.mc.gameSettings.anaglyph) GlStateManager.translate((pass * 2 - 1) * 0.1F, 0.0F, 0.0F);
	...
	public void updateCameraAndRender(final float partialTicks, final long nanoTime) {
		...
		this.mc.mcProfiler.endSection();
		if (this.mc.thePlayer != null && this.mc.theWorld != null) this.updateZoom();
		// ColdPlay >>> EventRender (per-frame, frame-accurate rotation for AimAssist)
		if (this.mc.thePlayer != null && this.mc.theWorld != null) {
			coldplay.ColdPlay.post(new coldplay.event.EventRender(partialTicks));
		}
		// ColdPlay <<<
	...
	private void renderWorldPass(final int pass, final float partialTicks, final long finishTimeNano) {
		...
		final boolean flag1 = !isRenderingMirror() && this.isDrawBlockOutline();
		GlStateManager.enableCull();
		this.mc.mcProfiler.endStartSection("clear");
		if (flag) Shaders.setViewport(0, 0, this.mc.displayWidth, this.mc.displayHeight);
		else GlStateManager.viewport(0, 0, worldWidth(), worldHeight());
		...
		if (isRenderingMirror()) ActiveRenderInfo.updateRenderInfo(mirrorPitch, mirrorYaw, false);
		else ActiveRenderInfo.updateRenderInfo(this.mc.thePlayer, this.mc.gameSettings.thirdPersonView == 2);
		...
		if (!isRenderingMirror()) this.checkLoadVisibleChunks(entity, partialTicks, icamera, this.mc.thePlayer.isSpectator());
		...
		if (!isRenderingMirror() && (pass == 0 || pass == 2)) {
		...
		if (!isRenderingMirror() && this.mc.gameSettings.ofSmoothFps && pass > 0) {
		...
		if (!this.debugView) {
			...
			if (isRenderingMirror()) effectrenderer.renderLitParticles(entity, partialTicks, mirrorYaw, mirrorPitch);
			else effectrenderer.renderLitParticles(entity, partialTicks);
			RenderHelper.disableStandardItemLighting();
			this.setupFog(0, partialTicks);
		...
		// ColdPlay >>> composite Outline/Chams here, before the 3D overlays: blitted after renderWorld, the glow
		// models covered anything drawn inside their silhouette (KillAura's aim marker sits inside the target).
		renderglobal.renderEntityOutlineFramebuffer();
		// ColdPlay >>> EventRender3D (world space, after terrain/entities/translucent; fire once, skip anaglyph double-dispatch)
		if (isRenderingMirror()) {
			postMirror(EventRenderMirror.Stage.WORLD, partialTicks);
		} else if (pass == (this.mc.gameSettings.anaglyph ? 1 : 2)) {
			if (this.mc.gameSettings.anaglyph) GlStateManager.colorMask(true, true, true, false);
			try {
				coldplay.ColdPlay.post(new coldplay.event.EventRender3D(partialTicks));
			} finally {
				if (this.mc.gameSettings.anaglyph) GlStateManager.colorMask(true, false, false, false);
			}
		}
		// ColdPlay <<<
		if (!isRenderingMirror() && Reflector.ForgeHooksClient_dispatchRenderLast.exists()) {
		...
		if (!isRenderingMirror() && this.renderHand && !Shaders.isShadowPass) {
			if (flag) {
	...
	private void renderCloudsCheck(final RenderGlobal renderGlobalIn, final float partialTicks, final int pass) {
		if (this.mc.gameSettings.renderDistanceChunks >= 4 && !Config.isCloudsOff() && Shaders.shouldRenderClouds(this.mc.gameSettings)) {
			...
			GlStateManager.matrixMode(5889);
			GlStateManager.loadIdentity();
			Project.gluPerspective(this.getFOVModifier(partialTicks, true), (float) worldWidth() / (float) worldHeight(), 0.05F, this.clipDistance * 4.0F);
			GlStateManager.matrixMode(5888);
			GlStateManager.pushMatrix();
			this.setupFog(0, partialTicks);
	...
	private void updateFogColor(final float partialTicks) {
		...
		Vec3 vec3 = world.getSkyColor(this.getWorldViewEntity(), partialTicks);
		vec3 = CustomColors.getWorldSkyColor(vec3, world, this.getWorldViewEntity(), partialTicks);
		...
		vec31 = CustomColors.getWorldFogColor(vec31, world, this.getWorldViewEntity(), partialTicks);
		...
		if (this.cloudFog) {
			...
			final Vec3 vec35 = CustomColors.getUnderwaterColor(this.mc.theWorld, this.getWorldViewEntity().posX, this.getWorldViewEntity().posY + 1.0D, this.getWorldViewEntity().posZ);
			if (vec35 != null) {
			...
			final Vec3 vec34 = CustomColors.getUnderlavaColor(this.mc.theWorld, this.getWorldViewEntity().posX, this.getWorldViewEntity().posY + 1.0D, this.getWorldViewEntity().posZ);
			if (vec34 != null) {
