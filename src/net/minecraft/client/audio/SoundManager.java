        ...
        URLStreamHandler urlstreamhandler = new URLStreamHandler()
        {
            protected URLConnection openConnection(final URL p_openConnection_1_)
            {
                return new URLConnection(p_openConnection_1_)
                {
                    ...
                    {
                        // [SNDDBG] TEMPORARY: log what the resource manager returns on Thread-5.
                        try
                        {
                            net.minecraft.client.resources.IResource res =
                                Minecraft.getMinecraft().getResourceManager().getResource(p_148612_0_);
                            InputStream is = res.getInputStream();
                            System.out.println("[SNDDBG] resolved " + p_148612_0_
                                + " stream=" + (is == null ? "null" : is.getClass().getName())
                                + " avail=" + (is == null ? -1 : is.available()));
                            return new BufferedInputStream(is);
                        }
                        catch (Throwable t)
                        {
                            System.out.println("[SNDDBG] FAILED " + p_148612_0_ + " : " + t);
