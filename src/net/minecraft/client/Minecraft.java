...
public class Minecraft implements IThreadListener, IPlayerUsage {
     ...
     * another block.
     */
    public int rightClickDelayTimer; // ColdPlay: exposed for FastPlace
    ...
    private void startGame() throws LWJGLException {
        ...
        coldplay.ColdPlay.getInstance().init();
    ...
    private void createDisplay() throws LWJGLException {
        ...
        Display.setTitle(coldplay.ColdPlay.NAME + " V" + coldplay.ColdPlay.VERSION);
        try {
    ...
    private void drawSplashScreen() {
        ...
        GlStateManager.enableTexture2D();
        // ColdPlay: dark shader backdrop + wordmark instead of the mojang logo. Single frame by design —
        // startGame keeps loading synchronously after this, so nothing drawn here could animate anyway.
        coldplay.gui.BackgroundShader.draw(scaledresolution.getScaledWidth(), scaledresolution.getScaledHeight(),
                scaledresolution.getScaledWidth() * i, scaledresolution.getScaledHeight() * i);
        coldplay.util.font.Fonts.load();
        if (coldplay.util.font.Fonts.isLoaded()) {
            final float centerX = scaledresolution.getScaledWidth() / 2f;
            final float centerY = scaledresolution.getScaledHeight() / 2f - 8;
            coldplay.gui.BackgroundShader.drawWordmark(centerX, centerY);
            final String status = "Loading...";
            coldplay.util.font.Fonts.title.drawStringWithShadow(status,
                    centerX - coldplay.util.font.Fonts.title.getStringWidth(status) / 2f,
                    centerY + coldplay.util.font.Fonts.logo.getHeight() / 2f + 6,
                    coldplay.gui.BackgroundShader.MIST);
        }
        GlStateManager.disableLighting();
        GlStateManager.disableFog();
    ...
    private void shutdownColdPlay() {
        try {
            coldplay.ColdPlay.getInstance().shutdown();
        } catch (Throwable failure) {
            logger.error("Failed to shut down ColdPlay cleanly", failure);
        }
        try {
            ...
            logger.error("Failed to save restored game settings", failure);
        }
    }
    ...
    public void shutdown() {
        coldplay.ColdPlay.getInstance().saveConfig();
        this.running = false;
    }
    ...
    private void clickMouse() {
        if (this.leftClickCounter <= 0) {
            // ColdPlay >>> EventAttack, inside the gate so listeners only see clicks vanilla acts on
            coldplay.event.EventAttack cpAttack = new coldplay.event.EventAttack(this.objectMouseOver);
            coldplay.ColdPlay.post(cpAttack);
            if (cpAttack.isCancelled()) return;
            // ColdPlay <<<
            this.thePlayer.swingItem();
    ...
    private void rightClickMouse() {
        // ColdPlay >>> EventUse / right-click (cancellable)
        coldplay.event.EventUse cpUse = new coldplay.event.EventUse(this.objectMouseOver);
        coldplay.ColdPlay.post(cpUse);
        if (cpUse.isCancelled()) return;
        // ColdPlay <<<
    ...
    public void runTick() throws IOException {
        // ColdPlay >>> EventUpdate PRE
        coldplay.ColdPlay.post(new coldplay.event.EventUpdate(coldplay.event.EventPhase.PRE));
        // ColdPlay <<<
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput) {
            ...
            while (Mouse.next()) {
                ...
                // ColdPlay >>> EventMouse for real button events (i == -1 is a move/wheel-only event).
                if (i >= 0) coldplay.ColdPlay.post(new coldplay.event.EventMouse(i, Mouse.getEventButtonState()));
            ...
            while (Keyboard.next()) {
                ...
                if (Keyboard.getEventKeyState()) {
                    ...
                    if (this.currentScreen != null) this.currentScreen.handleKeyboardInput();
                        ...
                        // ColdPlay >>> EventKey: in-game key press (module keybinds, ClickGUI open key)
                        coldplay.ColdPlay.post(new coldplay.event.EventKey(k));
                        // ColdPlay <<<
        ...
        this.mcProfiler.endSection();
        // ColdPlay >>> EventUpdate POST
        coldplay.ColdPlay.post(new coldplay.event.EventUpdate(coldplay.event.EventPhase.POST));
        // ColdPlay <<<
    ...
    /** Swaps the active account session (used by ColdPlay's account manager). */
    public void setSession(Session sessionIn) {
        if (sessionIn != null) {
            this.session = sessionIn;
            logger.info("Switched user: " + this.session.getUsername());
        }
    }
