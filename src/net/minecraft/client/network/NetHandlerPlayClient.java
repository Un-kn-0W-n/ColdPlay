...
public class NetHandlerPlayClient implements INetHandlerPlayClient {
    ...
    // ColdPlay >>> BedTracker's terrain-ready gate: don't lock the own-bed guess while chunks stream in.
    ...
    public void handleJoinGame(S01PacketJoinGame packetIn) {
        ...
        coldplay.broker.InventoryTransactions.getInstance().reset();
        coldplay.broker.SwordBlock.getInstance().reset(false);
        this.netManager.getPlayerPackets().reset();
        // ColdPlay >>> joining a (new) world is a game transition (see handleRespawn).
        coldplay.broker.GameStateTracker.getInstance().markTransitionPending();
        // ColdPlay <<<
    ...
    public void handleEntityTeleport(S18PacketEntityTeleport packetIn) {
        ...
        if (entity != null) {
            ...
            // ColdPlay >>> BackTrack: PositionGuard may hold this resolved update back (serverPos stays live above)
            if (coldplay.broker.PositionGuard.getInstance().interceptTeleport(
                    entity, d0, d1, d2, f, f1, packetIn.getOnGround())) {
                return;
            }
            // ColdPlay <<<
    ...
    public void handleEntityMovement(S14PacketEntity packetIn) {
        ...
        if (entity != null) {
            ...
            // ColdPlay >>> BackTrack: PositionGuard may hold this resolved update back (accumulators stay live above;
            // receipt-time f/f1 are ignored at drain when the packet carried no look)
            if (coldplay.broker.PositionGuard.getInstance().interceptRelative(
                    entity, d0, d1, d2, f, f1, packetIn.func_149060_h(), packetIn.getOnGround())) {
                return;
            }
            // ColdPlay <<<
    ...
    public void addToSendQueue(Packet p_147297_1_) {
        this.netManager.preparePacket(p_147297_1_);
        // ColdPlay >>> a Blink sword block may hold this tick's packets
        if (coldplay.broker.SwordBlock.getInstance().hold(p_147297_1_)) {
            return;
        }
    ...
    public void handleRespawn(S07PacketRespawn packetIn) {
        ...
        coldplay.broker.SwordBlock.getInstance().reset(true);
        this.netManager.getPlayerPackets().reset();
        // ColdPlay >>> a death/dimension respawn is a game transition -- BedTracker re-opens its own-bed
        // capture window (pre-lock) and ModuleManager runs its Auto-Off sweep off this signal.
        coldplay.broker.GameStateTracker.getInstance().markTransitionPending();
        // ColdPlay <<<
    ...
    public void handleSetSlot(S2FPacketSetSlot packetIn) {
        ...
        if (packetIn.func_149175_c() == -1) {
            ...
            if (packetIn.func_149175_c() == 0 && packetIn.func_149173_d() >= 36 && packetIn.func_149173_d() < 45) {
                ...
                coldplay.broker.InventoryTransactions.getInstance().slotChanged(
                        entityplayer.inventoryContainer, packetIn.func_149173_d(), packetIn.func_149174_e());
                ...
                coldplay.broker.InventoryTransactions.getInstance().slotChanged(
                        entityplayer.openContainer, packetIn.func_149173_d(), packetIn.func_149174_e());
            }
        }
    }
    ...
    public void handleConfirmTransaction(S32PacketConfirmTransaction packetIn) {
        ...
        coldplay.broker.InventoryTransactions.getInstance().confirmed(
                container, packetIn.getActionNumber(), packetIn.func_148888_e());
    }
    ...
    public void handleWindowItems(S30PacketWindowItems packetIn) {
        ...
        if (packetIn.func_148911_c() == 0) {
            ...
            coldplay.broker.InventoryTransactions.getInstance().contents(entityplayer.inventoryContainer, packetIn.getItemStacks());
            ...
            coldplay.broker.InventoryTransactions.getInstance().contents(entityplayer.openContainer, packetIn.getItemStacks());
        }
    }
