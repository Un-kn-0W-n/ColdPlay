...
public abstract class EntityThrowable extends Entity implements IProjectile
{
    ...
    public EntityThrowable(World worldIn, EntityLivingBase throwerIn)
    {
        ...
        Vec3 spawn = ProjectilePhysics.spawnPosition(throwerIn, 1.0F);
        this.setPosition(spawn.xCoord, spawn.yCoord, spawn.zCoord);
        Vec3 direction = ProjectilePhysics.throwableDirection(
                this.rotationYaw, this.rotationPitch, this.getInaccuracy(), 0.4F);
        this.setThrowableHeading(direction.xCoord, direction.yCoord, direction.zCoord, this.getVelocity(), 1.0F);
    }
    ...
    {
        return (float)ProjectilePhysics.THROWABLE_SPEED;
    }
    ...
    {
        Vec3 heading = ProjectilePhysics.heading(new Vec3(x, y, z), velocity, inaccuracy, this.rand, false);
        this.motionX = heading.xCoord;
        this.motionY = heading.yCoord;
        this.motionZ = heading.zCoord;
        float f1 = MathHelper.sqrt_double(this.motionX * this.motionX + this.motionZ * this.motionZ);
        this.prevRotationYaw = this.rotationYaw = (float)(MathHelper.atan2(this.motionX, this.motionZ) * 180.0D / Math.PI);
        this.prevRotationPitch = this.rotationPitch = (float)(MathHelper.atan2(this.motionY, (double)f1) * 180.0D / Math.PI);
        this.ticksInGround = 0;
    }
    ...
    public void onUpdate()
    {
        ...
        final ProjectilePhysics.Profile profile = ProjectilePhysics.THROWABLE_PROFILE
                .withGravity((double)this.getGravityVelocity());
        ProjectilePhysics.StepResult physicsStep = ProjectilePhysics.step(
                this.worldObj,
                ProjectilePhysics.StepState.from(this),
                profile,
                this,
                new Predicate<Entity>()
                {
                    public boolean apply(Entity candidate)
                    {
                        return candidate.canBeCollidedWith()
                                && (candidate != entitylivingbase || EntityThrowable.this.ticksInAir >= 5);
                    }
                },
                !this.worldObj.isRemote,
                inWater);
        MovingObjectPosition movingobjectposition = physicsStep.getCollision();
        ...
        ProjectilePhysics.StepState integrated = ProjectilePhysics.integrate(
                ProjectilePhysics.StepState.from(this), profile, inWater);
        this.posX = integrated.getPosition().xCoord;
        this.posY = integrated.getPosition().yCoord;
        this.posZ = integrated.getPosition().zCoord;
        float f1 = MathHelper.sqrt_double(this.motionX * this.motionX + this.motionZ * this.motionZ);
        ...
        this.motionX = integrated.getMotion().xCoord;
        this.motionY = integrated.getMotion().yCoord;
        this.motionZ = integrated.getMotion().zCoord;
    ...
    {
        return (float)ProjectilePhysics.THROWABLE_GRAVITY;
    }
