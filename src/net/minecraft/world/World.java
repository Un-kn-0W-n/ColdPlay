...
public abstract class World implements IBlockAccess {
	...
	/**
	 * Returns whether the supplied entity bounding box has collision support one block below after
	 * applying the requested horizontal offset. This is the geometry used by vanilla safe-walk.
	 */
	public boolean hasSupportAtOffset(final Entity entityIn, final AxisAlignedBB bb, final double x, final double z) {
		return !this.getCollidingBoundingBoxes(entityIn, bb.offset(x, -1.0D, z)).isEmpty();
	}

	/** Convenience overload using the entity's current bounding box. */
	public boolean hasSupportAtOffset(final Entity entityIn, final double x, final double z) {
		return this.hasSupportAtOffset(entityIn, entityIn.getEntityBoundingBox(), x, z);
	}
