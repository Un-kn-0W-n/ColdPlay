...
public class ItemBlock extends Item
{
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ)
    {
        ...
        else if (worldIn.canBlockBePlaced(this.block, pos, false, side, (Entity)null, stack))
        {
            ...
            {
                // ColdPlay >>> EventBlockPlace: the local player's own placement lands here first as a
                // client-side prediction; the server's confirming S23 then applies an identical state
                // (setBlockState returns false), so the WorldClient seam never sees own placements.
                if (worldIn.isRemote)
                {
