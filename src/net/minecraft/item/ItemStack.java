...
import net.minecraft.entity.ai.attributes.AttributeModifierMath;
...
public final class ItemStack
{
    ...
    public List<String> getTooltip(EntityPlayer playerIn, boolean advanced)
    {
        ...
        if (!multimap.isEmpty() && (i1 & 2) == 0)
        {
            ...
            for (Entry<String, AttributeModifier> entry : multimap.entries())
            {
                ...
                double d0 = this.getAttributeModifierDisplayAmount(attributemodifier, EnumCreatureAttribute.UNDEFINED);
    ...
    /**
     * Amount displayed for one attribute modifier. Vanilla folds the creature-enchantment bonus into
     * the item's canonical modifier line, identified by its UUID.
     */
    public double getAttributeModifierDisplayAmount(AttributeModifier modifier, EnumCreatureAttribute creatureAttribute)
    {
        double amount = modifier.getAmount();
        if (Item.itemModifierUUID.equals(modifier.getID()))
        {
            amount += (double)EnchantmentHelper.getModifierForCreature(this, creatureAttribute);
        }
        return amount;
    }

    /**
     * Computes this stack's contribution to an attribute using vanilla operation 0/1/2 ordering.
     * The attribute's base value is removed from the result so callers can compare stacks directly.
     */
    public double getAttributeContribution(IAttribute attribute)
    {
        final double base = attribute.getDefaultValue();
        Multimap<String, AttributeModifier> modifiers = this.getAttributeModifiers();
        return AttributeModifierMath.evaluate(attribute, base,
                modifiers.get(attribute.getAttributeUnlocalizedName())) - base;
    }

    /** Item/attribute contribution plus the creature-specific melee enchantment bonus. */
    public double getAttackDamageContribution(EnumCreatureAttribute creatureAttribute)
    {
        return this.getAttributeContribution(SharedMonsterAttributes.attackDamage)
                + (double)EnchantmentHelper.getModifierForCreature(this, creatureAttribute);
    }
