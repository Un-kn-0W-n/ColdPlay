...
public class AxisAlignedBB {
	...
	public Vec3 getCenter() {
		return new Vec3((this.minX + this.maxX) * 0.5D,
				(this.minY + this.maxY) * 0.5D,
				(this.minZ + this.maxZ) * 0.5D);
	}

	/** Returns the nearest point in or on this box to {@code point}. */
	public Vec3 closestPoint(final Vec3 point) {
		return new Vec3(
				MathHelper.clamp_double(point.xCoord, this.minX, this.maxX),
				MathHelper.clamp_double(point.yCoord, this.minY, this.maxY),
				MathHelper.clamp_double(point.zCoord, this.minZ, this.maxZ));
	}

	/** Squared distance from {@code point} to this box (zero for points inside it). */
	public double squareDistanceTo(final Vec3 point) {
		return point.squareDistanceTo(this.closestPoint(point));
	}
